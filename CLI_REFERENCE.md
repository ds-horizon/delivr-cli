...
The Delivr CLI uses a powerful binary diff implementation (`bsdiff43`) for creating efficient patch updates. This significantly reduces the update size by sending only the changes between versions.
...
- Always use `--isPatch true` when releasing a patch update
- Include all necessary assets with the patch bundle
- Patches work best for small to medium changes
- For major changes, consider using full bundle updates
- The patch creation process is handled by the native `bsdiff43` binary
- Compression can be applied to further reduce patch size

For technical details about the binary diff implementation, see [bsdiff/README.md](./bsdiff/README.md).
